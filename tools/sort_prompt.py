...
def main():
    ...
    with open(args.log, encoding="utf-8", encoding="utf-8") as f:
        result_list = []
        for line in f:
    ...
    for seed in seed_result:
        ...
        # Load all prompts
        with open(os.path.join(args.prompt_dir, print_name, "{}-{}.txt".format(args.k, seed)), encoding="utf-8") as f:
            prompts = []
            for line in f:
                prompts.append(line.strip())

        # Write sorted prompts
        fsort = open(os.path.join(args.prompt_dir, print_name, "{}-{}.sort.txt".format(args.k, seed)), 'w', encoding="utf-8")
        fscore = open(os.path.join(args.prompt_dir, print_name, "{}-{}.score.txt".format(args.k, seed)), 'w', encoding="utf-8")
