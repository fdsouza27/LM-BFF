...
def main():
    ...
    for seed in seed_result:
        ...
        # Load all templates
        with open(os.path.join(args.template_dir, print_name, "{}-{}.txt".format(args.k, seed)), encoding="utf-8") as f:
            templates = []
            for line in f:
                templates.append(line.strip())

        # Write sorted templates
        fsort = open(os.path.join(args.template_dir, print_name, "{}-{}.sort.txt".format(args.k, seed)), 'w', encoding="utf-8")
        fscore = open(os.path.join(args.template_dir, print_name, "{}-{}.score.txt".format(args.k, seed)), 'w', encoding="utf-8")
